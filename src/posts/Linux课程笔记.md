...
- 参数：被查看的文件夹，不提供参数，表示查看<strong>当前</strong>工作目录
- -l，以列表形式查看
- -h，配合-l，以更加人性化的方式显示文件大小
- -a，all，列出全部文件，包含的隐藏文件
...
功能：创建文件夹 `Make Directorty`

语法：`mkdir [-p] 参数`

- 参数：被创建文件夹的路径
- 选项：-p，可选，可以一次性创建多个层级的目录
...
功能：查看<strong>命令</strong>的程序本体文件路径
...
+ 语法2： `find 起始路径 -size + | -n[KMG]`
  + `+ -`表示大于和小于
  + n表示大小数字
  + KMG表示大小单位，k(小写字母)表示KB,M表示MB,G表示GB 例：查找小于10KB的文件：`find / -size -10k`
...
![](https://cdn.jsdelivr.net/gh/czlifetime/img/Linux%20getent.png)
